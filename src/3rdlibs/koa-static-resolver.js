/**
 * @see https://www.npmjs.com/package/koa-static-resolver
 * 此文件中代码源自 @see https://www.npmjs.com/package/koa-static-resolver
 * 此文件做了如下调整：
 * 1. 新增 prefix 配置项，静态文件无需必须从根路径进行访问。对方法 handleItem 做了简单调整。
 * 2. 调整源文件的一些换行与格式
 */
const fs = require('fs');
...
const handleItem = async (ctx, options) => {
  ...
  if(prefix) {
    real_path = file_path.replace(prefix, '');
  }
  
  const target_path = path.normalize(decodeURIComponent(real_path));
